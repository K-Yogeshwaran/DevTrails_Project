...
function AdminDashboard() {
    ...
    const fetchAll = async () => {
        try {
            const [s, w, p, f, z, az] = await Promise.all([
                ...
                fetch("http://localhost:5001/api/zones").then(r => r.json()),
                fetch("http://localhost:5001/api/zones/active").then(r => r.json()).catch(() => ({})),
            ]);
    ...
    const checkServices = async () => {
        ...
        const [backend, trigger, ml] = await Promise.all([
            check("http://localhost:8080/api/workers/health"),
            check("http://localhost:5001/api/health"),
            check("http://localhost:5003/api/payout/health"),
        ]);
    ...
    const handleClaim = async (claimId, action) => {
        try {
            await fetch(`http://localhost:8080/api/admin/claims/${claimId}/${action}`, {
                method: "POST",
                headers: { Authorization: `Bearer ${localStorage.getItem("adminToken")}` },
            });
